...
# article_ref: https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5895181/
# modified by ysun @2023-02-01, only input output paramters is changed, leaving cal algorithm not changed
# Requirements:
...
#main function
CIBERSORT <- function(sig_matrix, mixture_file, perm=0, QN=TRUE, absolute=FALSE, abs_method='sig.score',sep="\t"){
    ...
    if (is.character(sig_matrix)){
        print(paste('read sig_matrix file',sig_matrix))
        X <- read.table(sig_matrix,header=T,sep='\t',row.names=1,check.names=F)
    }else if(is.data.frame(sig_matrix) || is.matrix(sig_matrix)){
        X = sig_matrix
    }else{
        stop('! wrong param sig_matrix')
    }
    if (is.character(mixture_file)){
        print(paste('read mixture_file file',mixture_file))
        Y <- read.table(mixture_file, header=T, sep=sep,check.names=F)
        ...
        Y <- Y[,-1]
    }else if(is.data.frame(mixture_file) || is.matrix(mixture_file)){
        Y = mixture_file
    }else{
        stop('! wrong param mixture_file')
    }
    ...
    if(QN == TRUE){
        tmpc <- colnames(Y)
        tmpr <- rownames(Y)
        Y <- normalize.quantiles.robust(Y)
        colnames(Y) <- tmpc
        rownames(Y) <- tmpr
    }
